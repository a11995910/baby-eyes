# Baby Eyes Save - Android护眼监测应用

## 项目简介
Baby Eyes Save是一款专为保护儿童视力健康而设计的Android应用程序。通过智能监测和提醒功能，帮助家长和孩子养成良好的用眼习惯。

## 项目结构
```
roboter/
├── app/                          # 主应用模块
│   ├── src/main/
│   │   ├── java/wjcom/example/inc/
│   │   │   ├── MainActivity.kt           # 主活动
│   │   │   ├── FirstFragment.kt          # 第一个片段
│   │   │   ├── SecondFragment.kt         # 第二个片段
│   │   │   ├── service/
│   │   │   │   └── EyeProtectionService.kt  # 护眼服务
│   │   │   ├── ui/
│   │   │   │   └── FloatingWindowManager.kt # 悬浮窗管理器
│   │   │   └── utils/
│   │   │       ├── AudioRecorderManager.kt  # 音频录制管理器
│   │   │       ├── DistanceCalculator.kt    # 距离计算工具
│   │   │       └── SharedPrefsManager.kt   # 偏好设置管理
│   │   ├── res/                     # 资源文件
│   │   │   ├── drawable/            # 图标和背景
│   │   │   ├── layout/              # 布局文件
│   │   │   ├── values/              # 字符串、颜色、尺寸
│   │   │   └── xml/                 # 配置文件
│   │   └── AndroidManifest.xml      # 应用清单
│   ├── src/test/                    # 单元测试
│   └── src/androidTest/             # 集成测试
├── gradle/                          # Gradle配置
├── build.gradle.kts                 # 项目构建配置
├── settings.gradle.kts              # 项目设置
└── README.md                        # 项目说明
```

## ✨ 主要功能

### 🎯 核心功能
- **实时距离检测**: 利用前置摄像头和ML Kit人脸检测技术，实时计算用户与屏幕的距离
- **智能提醒**: 当距离低于设定阈值时，自动弹出全屏警告蒙层
- **悬浮球控制**: 提供可拖动的悬浮球，方便快速控制和设置
- **后台监护**: 前台服务保证检测在后台和其他应用上方持续工作

### ⚙️ 个性化设置
- **距离阈值调节**: 支持20-50cm范围内自由调整警告距离
- **自定义提醒语**: 可设置个性化的警告文本（不超过20字）
- **语音播报**: 支持录制自定义语音提醒（最长4秒），警示时播放录音并显示喇叭图标
- **检测开关**: 一键启用/暂停护眼检测功能

### 🎙️ 语音功能 (新增)
- **语音录制**: 长按录音按钮录制个性化语音提醒
- **试听功能**: 录制完成后可试听录音效果
- **智能切换**: 启用语音播报时，警示框显示🔊图标替代文字
- **音频管理**: 完整的录音/播放生命周期管理

### ⚡ 节能优化 (新增)
- **按需启动**: 相机仅在检测时启动，避免常驻后台
- **智能暂停**: 连续30帧无人脸检测时自动暂停相机
- **自动恢复**: 暂停3秒后自动重新激活检测
- **省电效果**: 显著降低电池消耗，延长设备续航

## 🛠️ 技术实现

### 技术栈
- **开发语言**: Kotlin
- **最低支持**: Android 8.0 (API 26)
- **目标版本**: Android 15 (API 36)
- **相机框架**: CameraX
- **人脸检测**: Google ML Kit Face Detection
- **UI框架**: Material Design 3

### 核心算法
- **距离计算公式**: `distance = (focal_length × real_IPD) / pixel_IPD`
- **人眼间距(IPD)**: 平均值6.3cm
- **防抖机制**: 连续3帧检测到距离过近才触发警告
- **焦距估算**: 基于图像分辨率和视场角动态计算

## 📱 使用说明

### 首次使用
1. **权限授权**: 首次启动会引导授予相机权限、录音权限和悬浮窗权限
2. **启动检测**: 点击主界面的悬浮按钮启动护眼检测
3. **悬浮球**: 系统会显示一个可拖动的护眼球图标

### 功能操作
- **单击护眼球**: 打开设置面板，调整阈值、提醒语和语音播报
- **拖动护眼球**: 可将护眼球移动到屏幕任意位置
- **长按护眼球**: 显示移除选项，可停止检测服务
- **语音录制**: 在设置面板中长按录音按钮录制自定义语音（最长4秒）
- **试听录音**: 录制完成后可点击试听按钮验证效果

### 距离警告
- 当检测到距离低于设定阈值时，会弹出全屏警告蒙层
- **文字模式**: 显示自定义的提醒文本
- **语音模式**: 播放录制的语音并显示🔊喇叭图标
- 必须点击"知道了"按钮才能关闭警告（已取消自动消失）

## 🔧 开发和构建

### 环境要求
- Android Studio Hedgehog | 2023.1.1 或更新版本
- Android SDK 36
- Gradle 8.13
- JDK 11

### 构建步骤
```bash
# 克隆项目
git clone https://github.com/a11995910/baby-eyes-save.git
cd baby-eyes-save

# 编译项目
./gradlew assembleDebug

# 安装到设备 (需要连接设备)
./gradlew installDebug
```

### 项目结构
```
app/src/main/java/wjcom/example/inc/
├── MainActivity.kt                 # 主Activity，权限管理和服务控制
├── service/
│   └── EyeProtectionService.kt    # 前台服务，相机和检测逻辑
├── ui/
│   └── FloatingWindowManager.kt   # 悬浮窗管理器
└── utils/
    ├── AudioRecorderManager.kt    # 音频录制管理器
    ├── DistanceCalculator.kt      # 距离计算算法
    └── SharedPrefsManager.kt      # 配置管理
```

## 🛡️ 隐私和安全

- **本地处理**: 所有图像分析均在设备本地进行，不会上传任何数据
- **实时丢弃**: 相机帧数据用完即丢，不做任何存储
- **权限说明**: 
  - 相机权限：用于人脸检测和距离计算
  - 录音权限：用于录制自定义语音提醒
  - 悬浮窗权限：用于显示护眼球和警告提醒
  - 前台服务权限：保证后台检测功能

## 📋 注意事项

- 请确保前置摄像头没有被遮挡
- 在光线充足的环境下使用效果更佳
- 建议定期清洁前置摄像头镜头
- 不同设备的相机参数可能影响距离计算精度
- 录制语音提醒时请在安静环境下进行
- 语音文件仅本地存储，不会上传到任何服务器

## 🎯 适用人群

- 有儿童的家长及监护人
- 关注用眼健康的个人用户
- 长时间使用手机的工作人群
- 教育机构和家庭用户

## 📱 系统要求

- Android 8.0 (API 26) 及以上版本
- 具备前置摄像头的设备
- 支持悬浮窗权限的系统

## 🔄 版本历史

### v2.0.0 (最新)
- 🎙️ **新增自定义语音播报功能**
  - 支持录制最长4秒的个性化语音提醒
  - 长按录音按钮进行录制
  - 试听功能验证录音效果
  - 警示时播放录音并显示🔊喇叭图标
- ⚡ **相机节能优化**
  - 按需启动相机，避免常驻后台
  - 连续30帧无人脸时自动暂停相机
  - 3秒后自动重新激活检测
  - 显著降低电池消耗
- 🔧 **技术改进**
  - 新增录音权限管理
  - 完善权限请求流程
  - 修复横屏人脸检测问题
  - 优化警示弹窗交互

### v1.0.0
- 基础护眼检测功能
- 可调节距离阈值和提醒语
- 悬浮球操作界面
- 全屏警告提醒

## 📞 联系支持

- 项目地址：https://github.com/a11995910/baby-eyes-save
- 问题反馈：请在 GitHub Issues 中提交
- 邮箱联系：[可选择性填写]

## 📄 开源协议

本项目遵循 MIT 开源协议。

## 🤝 贡献

欢迎提交 Issue 和 Pull Request 来改进这个项目。

### 贡献指南
1. Fork 本项目
2. 创建你的特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交你的修改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 打开一个 Pull Request

## ⭐ 致谢

感谢以下技术和库的支持：
- Google ML Kit Face Detection API
- CameraX 库
- Material Design 3
- Android Jetpack 组件

---

**让我们一起关爱眼睛健康！** 👀✨